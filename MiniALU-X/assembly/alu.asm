...
.prog_load:
    lea     rsi, [rel prog_load_prompt]
    mov     edx, prog_load_prompt_len
    call    write_stdout
    call    read_line_prog
    lea     rsi, [rel inbuf]
    call    parse_hex_bytes         ; EAX = count, CPU_LOAD_CAPPED set if needed
    test    eax, eax
    jz      .prog_loop              ; empty input => no change

    ; reset CPU state for new program
    mov     byte [rel CPU_PROG_LEN], al
    ...
    lea     rsi, [rel prog_loaded_prefix]
    mov     edx, prog_loaded_prefix_len
    call    write_stdout
    ; print count (still in EAX)
    call    print_u32
    lea     rsi, [rel prog_loaded_suffix]
    mov     edx, prog_loaded_suffix_len
    call    write_stdout

    movzx   eax, byte [rel CPU_LOAD_CAPPED]
    test    eax, eax
    jz      .prog_loop
    lea     rsi, [rel prog_loaded_capped]
    mov     edx, prog_loaded_capped_len
    call    write_stdout
    jmp     .prog_loop
...
.rca4_loop:
    ...
    ; Recompute X right before printing (print helpers clobber r11/r8/r9)
    mov     eax, ebx
    mov     cl,  r12b
    shr     eax, cl
    and     eax, 1
...
.mps_go:
    ...
    mov     edx, uop_oob_len
    call    write_stdout
    ; Control word + pipeline for OOB/halt: keep it short
    lea     rsi, [rel cw_oob]
    mov     edx, cw_oob_len
    call    write_stdout
    lea     rsi, [rel pipe_if_prefix]
    mov     edx, pipe_if_prefix_len
    call    write_stdout
    lea     rsi, [rel pipe_oob]
    mov     edx, pipe_oob_len
    call    write_stdout
    ret
.mps_fetch:
    ; FETCH
...
; NEW: Control word + pipeline helpers for each opcode (print after DECODE, before EXEC)
.mps_cw_load:
    lea     rsi, [rel cw_load]
    mov     edx, cw_load_len
    call    write_stdout
...
.mps_load:
    ...
    call    print_nl
    ; PIPE
    call    mini_pipe_print_if
    call    mini_pipe_print_id
    call    mini_pipe_print_ex_load
    ret
...
.mps_ld:
    ...
    call    print_nl
    ; PIPE
    mov     byte [rel CPU_EX_VAL], bl
    call    mini_pipe_print_if
    call    mini_pipe_print_id
    call    mini_pipe_print_ex_ld
    ret
